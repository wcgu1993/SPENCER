...
def convert_examples_to_features(example,tokenizer,args):
    ...
    nl_tokens = tokenizer.tokenize(example['nl'])[:args.input_length-5]
    code_tokens = tokenizer.tokenize(example['code'])[:args.input_length-len(nl_tokens)-5]
    ...
    return InputFeatures(tokens,ids,input_mask,example['label'])
...
class TextDataset(Dataset):
    ...
    def __getitem__(self, i):   
        return (torch.tensor(self.examples[i].ids),torch.tensor(self.examples[i].input_mask),torch.tensor(self.examples[i].label))
...
def evaluate(args, model, tokenizer, file_name, mode='dev', output=None):
    dataset = TextDataset(tokenizer, args, file_name)
    ...
    if args.do_test:
        ...
        for root, dirs, files in os.walk(test_data_dir):
            for file in files:
                ...
                result = evaluate(args, model, tokenizer, data_file, mode='test', output=file)
                logger.info("***** Eval results *****")
            for key in sorted(result.keys()):
