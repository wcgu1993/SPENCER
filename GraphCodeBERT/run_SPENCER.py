...
def train(args, model, teacher_model, distilled_model, tokenizer, pool, saved_dir):
    """ Train the model """
    ...
    for idx in range(args.num_train_epochs): 
        ...
        #evaluate    
        results = evaluate(args, model, distilled_model, tokenizer, pool, eval_when_training=True)
...
def evaluate(args, model, distilled_model, tokenizer, pool, eval_when_training=False):
    dataset = TextDataset(tokenizer, args, args.test_data_file, pool)
...
def main():
    ...
    if args.do_train:
        ...
        ta_model = train(args, model, model, distilled_model, tokenizer, pool, saved_dir)
        teacher_model = model
        results = evaluate(args, model, distilled_model, tokenizer, pool)
        original_mrr = results['mrr']
        distilled_results = evaluate(args, model, distilled_model, tokenizer, pool)
        ...
        while config.num_hidden_layers > 1 and distilled_mrr / original_mrr < 0.01:
            ...
            teacher_distilled_model = train(args, model, teacher_model, distilled_model, tokenizer, pool, teacher_saved_dir)

            ta_saved_dir = os.path.join(args.output_dir, "model_{}_to_{}_layer".format(ta_layer_num, config.num_hidden_layers))
            ta_distilled_model = train(args, model, ta_model, distilled_model, pool, tokenizer, ta_saved_dir)
            teacher_results = evaluate(args, model, teacher_distilled_model, tokenizer, pool)
            teacher_mrr = teacher_results['mrrs']
            ta_results = evaluate(args, model, ta_distilled_model, tokenizer, pool)
    ...
    if args.do_eval:
        ...
        result = evaluate(args, model, query_model, tokenizer, pool)
